...
function testGeminiDirectly(customApiKey = null) {
  chrome.storage.local.get(['aiModels'], async (result) => {
    ...
    const endpoint = flowAuto.endpoint || 'https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent';
    ...
    case 'generateTest':
      ...
      try {
        ...
        chrome.storage.local.get(['aiModels', 'userInstructions'], async (result) => {
          ...
          // Construir prompt con instrucciones y acciones
          let prompt = 'Genera un test automatizado Playwright siguiendo estas instrucciones:\n';
          if (instructions.custom_instructions) {
            prompt += `Instrucciones adicionales: ${instructions.custom_instructions}\n`;
            BackgroundLogger.log('debug', 'TEST_GEN', 'Added custom instructions', { 
              instructionsLength: instructions.custom_instructions.length 
            });
          } else {
            BackgroundLogger.log('debug', 'TEST_GEN', 'No custom instructions found');
          }
          prompt += `Acciones capturadas:\n`;
          request.actions.forEach((act, idx) => {
            prompt += `${idx + 1}. ${act.type} - selector: ${act.selector || ''} - valor: ${act.value || ''}\n`;
          });
          ...
          try {
            ...
            BackgroundLogger.log('debug', 'TEST_GEN', 'Making API request', {
              endpoint,
              promptLength: prompt.length,
            ...
            if (generated && generated.trim().length > 0) {
              BackgroundLogger.log('success', 'TEST_GEN', 'Test generated with Gemini', generated);
