...
// ===================================
// 📦 IMPORTS - SERVICIOS MODULARES
// ===================================
// Importar StateService para reemplazar GlobalState
async function importStateService() {
  try {
    const { stateService } = await import('./src/background/services/state-service.js');
    return stateService;
  } catch (error) {
    console.error('❌ Error importing StateService:', error);
    // Fallback temporal al GlobalState legacy
    return null;
  }
}

// Importar BadgeManager para reemplazar BadgeManager object
async function importBadgeManager() {
  try {
    const { BadgeManager } = await import('./src/badge/badge-manager.js');
    return new BadgeManager();
  } catch (error) {
    console.error('❌ Error importing BadgeManager:', error);
    // Fallback temporal al BadgeManager object legacy
    return null;
  }
}
...
(async () => {
  stateServiceInstance = await importStateService();
  badgeManagerInstance = await importBadgeManager();
  
  if (stateServiceInstance) {
    console.log('✅ StateService loaded successfully');
  } else {
    console.warn('⚠️ Using legacy GlobalState');
  }
  
  if (badgeManagerInstance) {
    console.log('✅ BadgeManager class loaded successfully');
  } else {
    console.warn('⚠️ Using legacy BadgeManager object');
  }
})();

// ===================================
// 🌐 ESTADO GLOBAL - MIGRADO A SERVICIOS MODULARES
// ===================================
// TODO: MIGRACIÓN PENDIENTE - Este GlobalState debe ser eliminado
// - Usar src/background/services/state-service.js en su lugar
// - El StateService ya implementa toda esta funcionalidad
// - Mantener solo por compatibilidad temporal hasta migración completa
...
// 🔢 BADGE MANAGER - DUPLICADO CON src/badge/badge-manager.js
// TODO: ELIMINAR - Esta implementación está duplicada
// - Usar src/badge/badge-manager.js en su lugar  
// - La clase BadgeManager ya implementa toda esta funcionalidad con mejor arquitectura
// - Mantener solo por compatibilidad temporal hasta migración completa
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  switch (request.action) {
    case 'getGlobalState':
      // ✅ MIGRADO A STATE-SERVICE
      // 🌐 Devolver estado global único
      (async () => {
        try {
          const state = stateServiceInstance 
            ? await stateServiceInstance.getState() 
            : GlobalState.getState(); // Fallback legacy
          sendResponse({ 
            success: true,
            state: state
          });
          ...
          sendResponse({ 
            success: false, 
            error: error.message,
            state: GlobalState.getState() // Fallback legacy
          });
        }
      })();
    ...
    case 'resetGlobalState':
      // ✅ MIGRADO A STATE-SERVICE
      // 🧹 Resetear completamente el estado global
      BackgroundLogger.log('info', 'GLOBAL_STATE', 'Resetting global state');
      (async () => {
        try {
          if (stateServiceInstance) {
            await stateServiceInstance.reset();
          } else {
            await GlobalState.reset(); // Fallback legacy
          }
          BackgroundLogger.log('success', 'GLOBAL_STATE', 'Global state reset complete');
      ...
      chrome.storage.local.get(['capturedSteps'], (result) => {
        ...
        chrome.storage.local.set({ capturedSteps: steps }, () => {
          if (chrome.runtime.lastError) {
            ...
            // ✅ MIGRADO A BADGE-SERVICE
            // 🔢 Actualizar badge con el contador de pasos
            try {
              if (badgeManagerInstance) {
                badgeManagerInstance.updateBadge(steps.length, 'RECORDING');
              } else {
                BadgeManager.updateBadge(steps.length); // Fallback legacy
              }
            } catch (error) {
    ...
    case 'saveSessionState':
      ...
      return true;  // ✅ CRÍTICO: Necesario para operaciones asíncronas
    ...
    case 'clearSession':
      chrome.storage.local.set({ 
        ...
        if (chrome.runtime.lastError) {
          ...
          // ✅ MIGRADO A BADGE-SERVICE - Limpiar badge al limpiar sesión
          try {
            if (badgeManagerInstance) {
              badgeManagerInstance.clearBadge();
            } else {
              BadgeManager.clearBadge(); // Fallback legacy
            }
          } catch (error) {
    ...
    case 'startRecording':
      ...
      // ✅ MIGRADO A STATE-SERVICE - VALIDACIÓN CRÍTICA: Verificar si ya hay grabación activa
      (async () => {
        try {
          const currentState = stateServiceInstance 
            ? await stateServiceInstance.getState() 
            : GlobalState.getState();
          if (currentState.isRecording) {
        BackgroundLogger.log('warning', 'RECORDING', 'Recording already active, rejecting new recording request');
        sendResponse({ 
          success: false, 
          error: 'Ya hay una grabación activa. Detén la grabación actual primero.',
          isRecording: true
        });
        return true;
      }
      
      // NO re-inyectar content script - ya está cargado via manifest.json
      chrome.tabs.query({active: true, currentWindow: true}, (tabs) => {
        if (tabs[0] && isValidUrl(tabs[0].url)) {
          BackgroundLogger.log('debug', 'RECORDING', 'Enviando startRecording a content script existente', {tabId: tabs[0].id, url: tabs[0].url});
          
          // Intentar enviar mensaje primero (content script ya debe estar cargado)
          chrome.tabs.sendMessage(tabs[0].id, {
            ...
            // ✅ MIGRADO A STATE-SERVICE - Actualizar estado
            if (stateServiceInstance) {
              await stateServiceInstance.startRecording(request.sessionId, null, tabs[0].id, tabs[0].url);
            } else {
              await GlobalState.startRecording(request.sessionId, null, tabs[0].id, tabs[0].url); // Fallback legacy
            }
            
            // ✅ MIGRADO A BADGE-SERVICE - Sincronizar badge al iniciar grabación
            try {
              if (badgeManagerInstance) {
                await badgeManagerInstance.syncFromStorage();
              } else {
                BadgeManager.syncBadgeFromStorage(); // Fallback legacy
              }
            } catch (error) {
            ...
            // Solo si falla, inyectar como fallback
            BackgroundLogger.log('warning', 'RECORDING', 'Content script not ready, injecting as fallback...', error);
            
            chrome.scripting.executeScript({
              target: { tabId: tabs[0].id },
              files: ['src/content-enhanced-v2.js']
            }).then(() => {
              ...
              BackgroundLogger.log('success', 'RECORDING', 'Recording started successfully (after injection)', response);
              
              // Guardar estado de grabación en sessionData
              chrome.storage.local.set({
                sessionData: {
                  isRecording: true,
                  sessionId: request.sessionId,
                ...
                BackgroundLogger.log('success', 'RECORDING', 'Recording state saved to storage (after injection)');
              });
              
              sendResponse({ success: true });
            }).catch((injectionError) => {
              BackgroundLogger.log('error', 'RECORDING', 'Failed to start recording', injectionError);
              ...
              } else if (injectionError.message && injectionError.message.includes('Content Security Policy')) {
                errorMessage = 'Cannot record on this page - Content Security Policy restrictions';
              } else if (injectionError.message && injectionError.message.includes('Receiving end does not exist')) {
      ...
      chrome.tabs.query({active: true, currentWindow: true}, async (tabs) => {
        if (tabs[0] && isValidUrl(tabs[0].url)) {
          chrome.tabs.sendMessage(tabs[0].id, {
            ...
            BackgroundLogger.log('debug', 'RECORDING', 'Content script responded to stopRecording', contentResponse);
            
            // ✅ MIGRADO A STATE-SERVICE - Actualizar estado
            if (stateServiceInstance) {
              await stateServiceInstance.stopRecording();
            } else {
              await GlobalState.stopRecording(); // Fallback legacy
            }
            
            // 🔢 Mantener badge visible pero cambiar color a gris (grabación detenida)
            chrome.storage.local.get(['capturedSteps'], (result) => {
              const steps = result.capturedSteps || [];
              if (steps.length > 0) {
                const displayText = steps.length > 999 ? '999+' : steps.length.toString();
                chrome.action.setBadgeText({ text: displayText });
                chrome.action.setBadgeBackgroundColor({ color: '#9E9E9E' }); // Gris = detenido
                BackgroundLogger.log('debug', 'BADGE_MANAGER', `Badge updated to gray (stopped): ${displayText} steps`);
              }
            });
            
            // Esperar 500ms para que content script guarde en storage
            return new Promise(resolve => setTimeout(resolve, 500));
          }).then(() => {
            BackgroundLogger.log('success', 'RECORDING', 'Recording stopped successfully (after save delay)');
            sendResponse({ success: true });
          }).catch((error) => {
    ...
    case 'closeTestWindow':
      // 🚪 Cerrar ventana de Test Launcher activa
      BackgroundLogger.log('info', 'TEST_LAUNCHER', 'Close test window requested');
      
      (async () => {
        const windowId = stateServiceInstance 
          ? await stateServiceInstance.closeTestWindow()
          : await GlobalState.closeTestWindow(); // Fallback legacy
        
        if (windowId) {
          try {
            await chrome.windows.remove(windowId);
            // También eliminar de testWindows Map
            testWindows.delete(windowId);
            BackgroundLogger.log('success', 'TEST_LAUNCHER', `Test window closed: ${windowId}`);
            sendResponse({ success: true, windowId });
          } catch (error) {
            BackgroundLogger.log('warning', 'TEST_LAUNCHER', `Window already closed: ${windowId}`, error);
            sendResponse({ success: true, windowId, alreadyClosed: true });
          }
        } else {
          BackgroundLogger.log('info', 'TEST_LAUNCHER', 'No active test window to close');
          sendResponse({ success: true, noWindow: true });
        }
      })();
      return true;
...
`;
    ...
    case 'uploadToN8n':
      BackgroundLogger.log('info', 'N8N_UPLOAD', 'Upload to N8n webhook requested', {
        sessionDataPresent: !!request.sessionData,
        actionsCount: request.sessionData?.actions?.length || 0
      });
      
      try {
        // Validar que tenemos datos de sesión
        if (!request.sessionData) {
          BackgroundLogger.log('error', 'N8N_UPLOAD', 'No session data provided');
          sendResponse({ success: false, error: 'No session data to upload' });
          return;
        }

        // Enviar directamente al webhook específico
        const webhookUrl = 'http://localhost:5678/webhook-test/testbuiler/record';
        const uploadData = {
          sessionId: request.sessionData.sessionId || `session_${Date.now()}`,
          url: request.sessionData.url || '',
          title: request.sessionData.title || 'TestBuilder Session',
          timestamp: Date.now(),
          actions: request.sessionData.actions || [],
          metadata: {
            userAgent: navigator.userAgent,
            timestamp: new Date().toISOString(),
            source: 'TestBuilder-Extension',
            type: 'on-demand-upload'
          }
        };

        BackgroundLogger.log('debug', 'N8N_UPLOAD', 'Sending data to webhook', {
          url: webhookUrl,
          sessionId: uploadData.sessionId,
          actionsCount: uploadData.actions.length
        });
        ...
        fetch(webhookUrl, {
          ...
          headers: {
            'Content-Type': 'application/json',
            'User-Agent': 'TestBuilder-Extension/1.0'
          },
        ...
        .then(async response => {
          if (!response.ok) {
            const errorText = await response.text();
            BackgroundLogger.log('error', 'N8N_UPLOAD', 'Webhook request failed', {
              status: response.status,
              statusText: response.statusText,
              error: errorText
            });
          ...
          const result = await response.json();
          BackgroundLogger.log('success', 'N8N_UPLOAD', 'Upload to N8n successful', {
            result,
            sessionId: uploadData.sessionId
          });
          ...
          sendResponse({ 
            ...
            result,
            sessionId: uploadData.sessionId,
            message: 'Session uploaded successfully to N8n workflow'
          });
        })
        .catch(error => {
          BackgroundLogger.log('error', 'N8N_UPLOAD', 'Upload to N8n failed', error);
          sendResponse({ 
            success: false, 
        ...
        BackgroundLogger.log('error', 'N8N_UPLOAD', 'Unexpected error in N8n upload', error);
        sendResponse({ success: false, error: error.message });
      }
      
      return true; // Keep message channel open for async response
      
    case 'launchIsolatedTest':
      BackgroundLogger.log('info', 'TEST_LAUNCHER', 'Launching isolated test window', request.options);
      launchIsolatedTestWindow(request.options)
        .then(result => {
          BackgroundLogger.log('success', 'TEST_LAUNCHER', 'Isolated window created successfully', result);
          sendResponse(result);
        })
        .catch(error => {
          BackgroundLogger.log('error', 'TEST_LAUNCHER', 'Failed to create isolated window', error);
          sendResponse({ success: false, error: error.message });
        });
      return true; // Keep message channel open for async response
      
    case 'getTestWindows':
      const windows = Array.from(testWindows.entries()).map(([windowId, data]) => ({
        windowId,
        ...data
      }));
      BackgroundLogger.log('info', 'TEST_LAUNCHER', 'Returning test windows', { count: windows.length });
...
chrome.webNavigation.onCompleted.addListener((details) => {
  if (details.frameId === 0) {
    ...
    chrome.storage.local.get(['sessionData'], (result) => {
      ...
      // 🐛 DEBUG: Ver qué sessionData se está leyendo
      console.log('🔍 [DEBUG NAVIGATION] sessionData:', result.sessionData);
      console.log('🔍 [DEBUG NAVIGATION] isRecording:', result.sessionData?.isRecording);
      console.log('🔍 [DEBUG NAVIGATION] sessionId:', result.sessionData?.sessionId);
        ...
        setTimeout(() => {
          safeScriptInjection(details.tabId, (tab) => {
            chrome.scripting.executeScript({
              ...
              // Re-iniciar grabación en la nueva página (sin resetear acciones)
              if (result.sessionData && result.sessionData.sessionId) {
                setTimeout(() => {
                  chrome.tabs.sendMessage(details.tabId, {
                    action: 'continueRecording',  // ✅ Cambio: continuar en lugar de start
                    sessionId: result.sessionData.sessionId
                  }).catch(err => {
                    BackgroundLogger.log('error', 'NAVIGATION', 'Failed to continue recording on new page', err);
                  });
                }, 500);
              }
            }).catch(err => {
...
// ===================================
// 🚀 TEST LAUNCHER - Ventanas Aisladas
// ===================================

// Gestión de ventanas de test aisladas
const testWindows = new Map(); // windowId -> { sessionId, url, startTime, options }

// Función para crear ventana aislada
async function launchIsolatedTestWindow(options) {
  const { url, incognito, fullscreen, autostart } = options;
  
  BackgroundLogger.log('info', 'TEST_LAUNCHER', `Iniciando ventana aislada para: ${url}`, options);
  
  try {
    // Guardar última URL usada
    await chrome.storage.local.set({ lastTestUrl: url });
    
    // Si se solicita incógnito, verificar que esté habilitado
    if (incognito) {
      const isIncognitoAllowed = await chrome.extension.isAllowedIncognitoAccess();
      if (!isIncognitoAllowed) {
        BackgroundLogger.log('warning', 'TEST_LAUNCHER', 'Incógnito no habilitado, usando ventana normal');
        throw new Error('La extensión no tiene permiso para ventanas de incógnito. Ve a chrome://extensions/ y habilita "Permitir en modo incógnito".');
      }
    }
    ...
    BackgroundLogger.log('debug', 'TEST_LAUNCHER', 'Creando ventana con configuración:', windowConfig);
    
    // Crear ventana
    const window = await chrome.windows.create(windowConfig);
    
    if (!window || !window.id || !window.tabs || !window.tabs[0]) {
      throw new Error('No se pudo crear la ventana. La respuesta de chrome.windows.create() es inválida.');
    }
    ...
    // Registrar ventana de test
    testWindows.set(windowId, {
      sessionId,
      url,
      tabId,
      startTime: Date.now(),
    ...
    BackgroundLogger.log('success', 'TEST_LAUNCHER', `Ventana creada: ${windowId}`, { sessionId, tabId });
    ...
    // Inyectar content script en la ventana de test
    await injectContentScriptInTestWindow(tabId, windowId);
    
    // Si autostart está activado, iniciar grabación automáticamente
    if (autostart) {
      setTimeout(async () => {
        try {
          ...
          // ✅ MIGRADO A STATE-SERVICE - Actualizar estado con ventana de test
          if (stateServiceInstance) {
            await stateServiceInstance.startRecording(sessionId, windowId, tabId, url);
          } else {
            await GlobalState.startRecording(sessionId, windowId, tabId, url); // Fallback legacy
          }
          
          BackgroundLogger.log('success', 'TEST_LAUNCHER', `Grabación iniciada automáticamente en ventana ${windowId}`);
          
          // Notificar al popup que el test launcher se activó
          chrome.runtime.sendMessage({
            type: 'TEST_LAUNCHER_STARTED',
            data: {
              sessionId,
              url,
              windowId,
          ...
          BackgroundLogger.log('warning', 'TEST_LAUNCHER', `No se pudo iniciar grabación automática: ${error.message}`);
        }
      }, 1000);
    }
    
    // Listener para detectar cuando se cierra la ventana
    const closeListener = (closedWindowId) => {
      if (closedWindowId === windowId) {
        BackgroundLogger.log('info', 'TEST_LAUNCHER', `Ventana de test cerrada: ${windowId}`);
        testWindows.delete(windowId);
        chrome.windows.onRemoved.removeListener(closeListener);
        
        // Notificar al popup que la ventana se cerró
        chrome.runtime.sendMessage({
          type: 'TEST_WINDOW_CLOSED',
          data: { windowId, sessionId }
        }).catch(() => {
          // Popup podría no estar abierto, ignorar error
        });
      }
    ...
    return {
      ...
      tabId,
      sessionId,
      message: 'Ventana aislada creada exitosamente'
    };
    
  } catch (error) {
    BackgroundLogger.log('error', 'TEST_LAUNCHER', `Error al crear ventana aislada: ${error.message}`, error);
    throw error;
  }
}

// Función para inyectar content script en ventana de test
async function injectContentScriptInTestWindow(tabId, windowId) {
  try {
    // Verificar que es una URL válida
    const tab = await chrome.tabs.get(tabId);
    
    if (!isValidUrl(tab.url)) {
      BackgroundLogger.log('warning', 'TEST_LAUNCHER', `URL no válida para inyección: ${tab.url}`);
      return;
    }
    
    // Inyectar content-enhanced-v2.js
    await chrome.scripting.executeScript({
      target: { tabId: tabId },
      files: ['src/content-enhanced-v2.js']
    });
    
    BackgroundLogger.log('success', 'TEST_LAUNCHER', `Content script inyectado en tab ${tabId} (ventana ${windowId})`);
    
  } catch (error) {
    BackgroundLogger.log('error', 'TEST_LAUNCHER', `Error al inyectar content script: ${error.message}`, error);
    throw error;
  }
}

// Limpiar ventanas de test al cerrar la extensión
chrome.runtime.onSuspend.addListener(() => {
  BackgroundLogger.log('info', 'TEST_LAUNCHER', `Limpiando ${testWindows.size} ventanas de test activas`);
  testWindows.clear();
});

// Comando de teclado para abrir el Test Launcher
chrome.commands.onCommand.addListener((command) => {
  if (command === 'open-launcher') {
    chrome.tabs.create({
      url: chrome.runtime.getURL('test-launcher.html')
    });
    BackgroundLogger.log('info', 'TEST_LAUNCHER', 'Test Launcher abierto desde comando de teclado');
  }
});

// ✅ MIGRADO A BADGE-SERVICE - Sincronizar badge al cargar el background script
// Esto restaura el contador si la extensión se recarga
(async () => {
  try {
    if (badgeManagerInstance) {
      await badgeManagerInstance.syncFromStorage();
      BackgroundLogger.log('success', 'BADGE_MANAGER', 'Badge sincronizado con BadgeManager class');
    } else {
      // Fallback legacy si el import falla
      BadgeManager.syncBadgeFromStorage().then(() => {
        BackgroundLogger.log('success', 'BADGE_MANAGER', 'Badge sincronizado con fallback legacy');
      });
    }
  } catch (error) {
    console.error('❌ Error syncing badge on startup:', error);
  }
})();
